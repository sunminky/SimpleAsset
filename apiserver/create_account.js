...
const main = async function main(name, deposit) {
    try {
        ...
        if (!userExists) {
            ...
            return `An identity for the user ${name} does not exist in the wallet\nRun the registerUser.js application before retrying`;
        }
        ...
        const result = await contract.submitTransaction("create_account", name, deposit);
