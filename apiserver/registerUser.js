...
const main = async function main(name) {
    try {
        ...
        if (userExists) {
            ...
            return `An identity for the user ${name} already exists in the wallet`;
        }
        ...
        return `Successfully registered and enrolled admin user ${name} and imported it into the wallet`;
    } catch (error) {
        console.error(`Failed to register user ${name}: ${error}`);
        // process.exit(1);
        return `Failed to register user ${name}: ${error}`;
    }
}
